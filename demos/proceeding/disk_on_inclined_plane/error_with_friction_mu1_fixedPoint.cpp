...
int main()
{
    // Figure 6: Disk falling on a plane with friction. L^2 error on the position of the center and the rotation of the disk as a function
    // of dt. mu = 1, fixed point algorithm
    plog::init(plog::warning, "error_with_friction_mu1_fixedPoint.log");
    ...
    for (std::size_t i = 0; i < dt.size(); ++i)
    {
        ...
        {
            scopi::ScopiSolver<dim, scopi::OptimMosek<scopi::DryWithFrictionFixedPoint>, scopi::contact_kdtree, scopi::vap_fpd> solver(
                particles,
                dt[i]);
            ...
            error_pos += (xt::linalg::norm(pos(1) - pos_analytical) / xt::linalg::norm(pos_analytical))
                       * (xt::linalg::norm(pos(1) - pos_analytical) / xt::linalg::norm(pos_analytical));

            auto q            = particles.q();
            auto q_analytical = scopi::quaternion(tmp.second);
            error_rot += (xt::linalg::norm(q(1) - q_analytical) / xt::linalg::norm(q_analytical))
                       * (xt::linalg::norm(q(1) - q_analytical) / xt::linalg::norm(q_analytical));
        }
