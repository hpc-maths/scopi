...
//     const const double pi = 4 * std::atan(1);
//     const double d1       = std::sqrt(2 * (1 - std::cos(theta_g - theta_d)));
//     const double xnew     = rx * (std::sin(theta_g) - std::sin(theta_d)) / d1;
//     const double ynew     = -ry * (std::cos(theta_g) - std::cos(theta_d)) / d1;
//     const double ynew2    = ynew * rx * ry / (std::pow(std::pow(ry * xnew, 2 / e) + std::pow(rx * ynew, 2 / e), e / 2));
//     const double sinb     = std::sqrt(std::pow(std::pow(ynew2 / ry, 2), 1 / e));
//     const double b        = std::asin(sinb);
//     binit.push_back(b);
//     binit.push_back(b + pi / 2);
//     binit.push_back(b + pi);
//     binit.push_back(b + 3 * pi / 2);
//     const double theta_milieu = 0.5 * (theta_g + theta_d);
