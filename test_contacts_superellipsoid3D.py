...
class SuperEllipsoid3D:
    ...
    def __init__(self, x,y,z, rx,ry,rz, n=1, e=1, theta=0, w=[0,0,1], q=None):
        """
...
if __name__ == '__main__':
    ...
    def create_ainit_yz(n, s, liste_a=None):
        ...
        for a_ell in angles:
            ...
            # print("y_supell =",y_supell," z_supell =",z_supell)
            ...
            liste_a.append(a-np.pi/2)
        liste_a = np.unique(liste_a)
        liste_a = (liste_a==0)*0.01 + (liste_a!=0)*liste_a
        return np.unique(liste_a)
    ...
    # s1 = SuperEllipsoid3D(-0.2,  0. ,  0.,   0.1 ,  0.05,  0.05, e=1, n=0.6,  q=[0.866025,  0.      ,  0.      ,  0.5])
    # s2 = SuperEllipsoid3D(0.21,  0.  ,  0.,    0.1 ,  0.05,  0.05, e=1, n=1, q = [0.92388 , -0.      , -0.      , -0.382683])

    # s1 = SuperEllipsoid3D(0.025916,  0.000908, -0.000013,   0.1 ,  0.05,  0.05, e=1, n=0.6,  q=[0.482958,  0.      ,  0.      ,  0.875644])
    # s2 = SuperEllipsoid3D(-0.015916, -0.000908,  0.000013,    0.1 ,  0.05,  0.05, e=1, n=1, q = [0.607465,  0.      ,  0.      , -0.794346])

    # s1 = SuperEllipsoid3D(-1.825000e-01, -2.664870e-13,  2.926989e-15,    0.1 ,  0.05,  0.05, e=1, n=0.6,  q=[0.866025,  0.      ,  0.      ,  0.5 ])
    # s2 = SuperEllipsoid3D( 1.925000e-01,  2.664870e-13, -2.926992e-15,    0.1 ,  0.05,  0.05, e=1, n=1, q = [0.92388 ,  0.      ,  0.      , -0.382683])

    # s1 = SuperEllipsoid3D(-1.750000e-01,  7.343024e-13,  6.335213e-14,    0.1 ,  0.05,  0.05, e=1, n=0.6,  q=[ 0.866025,  0.      ,  0.      ,  0.5])
    # s2 = SuperEllipsoid3D( 1.850000e-01, -7.343024e-13, -6.335213e-14,    0.1 ,  0.05,  0.05, e=1, n=1, q = [0.92388 ,  0.      ,  0.      , -0.382683])

    #it = 0
    # s1 = SuperEllipsoid3D(-0.20,  0. ,  0. ,    0.1 ,  0.05,  0.05, e=1, n=0.6,  q=[ 0.866025,  0.      ,  0.      ,  0.5  ])
    # s2 = SuperEllipsoid3D( 0.21,  0. ,  0.,     0.1 ,  0.05,  0.05, e=1, n=1, q = [ 0.92388 , -0.      , -0.      , -0.382683])
    ...
    num = 8
    binit1 = np.array(create_binit_xy(num, s1))
    ainit1 = np.array( create_ainit_yz(num, s1) )
    ...
    # print("ainit1_yz = ",ainit1_yz)
    # ainit1_xz = np.array( create_ainit_xz(num, s1) )
    # print("ainit1_xz = ",ainit1_xz)
    # ainit1 = np.concatenate((ainit1_yz,ainit1_xz))
    agrid1, bgrid1 = np.meshgrid(ainit1, binit1)
    # print("agrid1 = ",agrid1, "bgrid1 = ",bgrid1)
    agrid1 = agrid1.T.reshape((-1,))
    bgrid1 = bgrid1.T.reshape((-1,))
    # print("flatten agrid1 = ",agrid1, "flatten bgrid1 = ",bgrid1)
    points1 = s1.surface_pt(agrid1,bgrid1)
    normals1 = s1.surface_normal(agrid1,bgrid1)
    # points1 = s1.surface_pt(ainit1,binit1)
    # normals1 = s1.surface_normal(ainit1,binit1)
    binit2 = np.array(create_binit_xy(num, s2))
    ainit2 = np.array( create_ainit_yz(num, s2) )
    ...
    # print("ainit2_yz = ",ainit2_yz)
    # ainit2_xz = np.array( create_ainit_xz(num, s2) )
    # print("ainit2_xz = ",ainit2_xz)
    # ainit2 = np.concatenate((ainit2_yz,ainit2_xz))
    agrid2, bgrid2 = np.meshgrid(ainit2, binit2)
    agrid2 = agrid2.T.reshape((-1,))
    bgrid2 = bgrid2.T.reshape((-1,))
    # print("flatten agrid2 = ",agrid2, "flatten bgrid2 = ",bgrid2)
    points2 = s2.surface_pt(agrid2,bgrid2)
    normals2 = s2.surface_normal(agrid2,bgrid2)
    # points2 = s2.surface_pt(ainit2,binit2)
    # normals2 = s2.surface_normal(ainit2,binit2)
    distances = np.zeros((points1.shape[0],points2.shape[0]))
    print("points1.shape[0] : ",points1.shape[0], " binit1.shape : ",binit1.shape)
    print("nb calculs de distance : ",points1.shape[0]*points2.shape[0])
    for i in range(points1.shape[0]):
        for j in range(points2.shape[0]):
            distances[i,j] = np.linalg.norm(points1[i,:]-points2[j,:])  + 2*(1+np.dot(normals1[i,:],normals2[j,:])/(np.linalg.norm(normals1[i,:])*np.linalg.norm(normals2[j,:])))
    print("distances = ",distances)
    ...
    sign_distance = np.sign(np.dot( final_pt2[0,:]-final_pt1[0,:], normal_final_pt1[0,:] ))
