  ...
  class KdTree
  {
    ...
    void mosek_solver(scopi::scopi_container<dim>& particles, double dt, std::size_t total_it, std::size_t active_ptr)
    {
        ...
        std::cout << "\n\n------------------- Time iteration ----------------> " << nite << std::endl;
            ...
            xt::xtensor<double, 1> c = xt::zeros<double>({1 + 2*3*Nactive});
            c(0) = 1;
            std::size_t Mdec = 1;
            ...
            auto A = Matrix::sparse(contacts.size(), 1 + 6*Nactive,
            ...
            xt::xtensor<double, 2> sqrtM = xt::zeros<double>({6*Nactive, 6*Nactive});
            for (std::size_t i=0; i<Nactive; ++i)
            {
                for (std::size_t d=0; d<3; ++d)
                {
                    sqrtM(3*i+d, 3*i+d) = std::sqrt(mass); // TODO: add mass into particles
                    sqrtM(3*i+3*Nactive+d, 3*i+3*Nactive+d) = std::sqrt(moment);
                }
            }
            ...
            // variables
            Variable::t X = model->variable("X", 1 + 6*Nactive);
            ...
            Constraint::t qc2 = model->constraint("qc2", Expr::vstack(1, X->index(0), Expr::mul(sqrtM_mosek, X->slice(1, 1 + 6*Nactive))), Domain::inRotatedQCone());
