...
namespace scopi
{
                                   ...
                                   public gradient_t<problem_t>
    {
      ...
      protected:
        ...
        OptimProjectedGradient(std::size_t nparts, double dt, const scopi_container<dim>& particles);
      ...
      public:
         ...
         */
        template <std::size_t dim>
        std::size_t solve_optimization_problem_impl(const scopi_container<dim>& particles, const std::vector<neighbor<dim>>& contacts);
        /**
      ...
      private:
         ...
         */
        template <std::size_t dim>
        void create_matrix_B(const scopi_container<dim>& particles, const std::vector<neighbor<dim>>& contacts);
        /**
    ...
    template <class problem_t, template <class> class gradient_t>
    template <std::size_t dim>
    OptimProjectedGradient<problem_t, gradient_t>::OptimProjectedGradient(std::size_t nparts, double dt, const scopi_container<dim>& particles)
                                                                        ...
                                                                        const std::vector<neighbor<dim>>& contacts)
    {
        ...
        PLOG_ERROR_IF(m_status != SPARSE_STATUS_SUCCESS && m_status != SPARSE_STATUS_NOT_SUPPORTED)
            << "Error in mkl_sparse_set_mv_hint for matrix B SPARSE_OPERATION_NON_TRANSPOSE: " << m_status;
        m_status = mkl_sparse_set_mv_hint(m_B, SPARSE_OPERATION_TRANSPOSE, m_descrB, 1);
        PLOG_ERROR_IF(m_status != SPARSE_STATUS_SUCCESS && m_status != SPARSE_STATUS_NOT_SUPPORTED)
            << "Error in mkl_sparse_set_mv_hint for matrix B SPARSE_OPERATION_TRANSPOSE: " << m_status;
    ...
    void OptimProjectedGradient<problem_t, gradient_t>::create_matrix_A()
    {
        ...
        m_status           = mkl_sparse_sp2m(SPARSE_OPERATION_NON_TRANSPOSE,
                                   m_descr_inv_P,
                                   m_inv_P,
        ...
        PLOG_ERROR_IF(m_status != SPARSE_STATUS_SUCCESS && m_status != SPARSE_STATUS_NOT_SUPPORTED)
            << "Error in mkl_sparse_set_mv_hint for matrix A: " << m_status;
    ...
    OptimParams<OptimProjectedGradient<problem_t, gradient_t>>::OptimParams(
        const OptimParams<OptimProjectedGradient<problem_t, gradient_t>>& params)
