...
namespace scopi{
    ...
    template<std::size_t dim>
    void OptimMosek<dim>::create_matrix_constraint_impl(const std::vector<neighbor<dim>>& contacts)
    {
        // Preallocate
        ...
        m_A = Matrix::sparse(contacts.size(), 1 + 6*this->m_Nactive + 6*this->m_Nactive,
    ...
    template<std::size_t dim>
    void OptimMosek<dim>::create_matrix_mass_impl()
    {
        std::vector<int> Az_rows;
        std::vector<int> Az_cols;
        std::vector<double> Az_values;

        Az_rows.reserve(6*this->m_Nactive*2);
        Az_cols.reserve(6*this->m_Nactive*2);
        Az_values.reserve(6*this->m_Nactive*2);
        ...
        for (std::size_t i = 0; i < this->m_Nactive; ++i)
        {
            for (std::size_t d = 0; d < 2; ++d)
            {
                ...
                Az_rows.push_back(3*i + d);
                Az_cols.push_back(1 + 6*this->m_Nactive + 3*i + d);
                Az_values.push_back(-1.);
            }

            Az_rows.push_back(3*this->m_Nactive + 3*i + 2);
            Az_cols.push_back(1 + 3*this->m_Nactive + 3*i + 2);
            Az_values.push_back(std::sqrt(this->m_moment));

            Az_rows.push_back(3*this->m_Nactive + 3*i + 2);
            Az_cols.push_back( 1 + 6*this->m_Nactive + 3*this->m_Nactive + 3*i + 2);
            Az_values.push_back(-1);
        }

        m_Az = Matrix::sparse(6*this->m_Nactive, 1 + 6*this->m_Nactive + 6*this->m_Nactive,
    ...
    int OptimMosek<dim>::solve_optimization_problem_impl(const std::vector<neighbor<dim>>&)
    {
        ...
        // variables
        Variable::t X = model->variable("X", 1 + 6*this->m_Nactive + 6*this->m_Nactive);
        ...
        Constraint::t qc3 = model->constraint("qc3", Expr::vstack(1, X->index(0), X->slice(1 + 6*this->m_Nactive, 1 + 6*this->m_Nactive + 6*this->m_Nactive)), Domain::inRotatedQCone());
    ...
    {
        return xt::adapt(reinterpret_cast<double*>(m_Xlvl->raw()+1), {this->m_Nactive, 3UL});
    }

    template<std::size_t dim>
    auto OptimMosek<dim>::get_wadapt_impl()
    {
        return xt::adapt(reinterpret_cast<double*>(m_Xlvl->raw()+1+3*this->m_Nactive), {this->m_Nactive, 3UL});
    }
    ...
    template<std::size_t dim>
    void OptimMosek<dim>::free_memory_impl()
    {}
