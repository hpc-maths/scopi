    ...
    template<std::size_t dim, typename SolverType>
        void ScopiSolver<dim, SolverType>::solve(std::size_t total_it)
        {
            ...
            for (std::size_t nite=0; nite<total_it; ++nite)
            {
                ...
                auto massMatrix = _solver.createMatrixMass();
                auto c = _solver.createVectorC();
                auto distances = _solver.createVectorDistances(contacts);
                auto duration4 = toc();
                ...
                auto nbIter = _solver.solveOptimizationProbelm(contacts, constraintMatrix, massMatrix, c, distances, sol);
                auto duration5 = toc();
