...
namespace scopi
{
    ...
    class OptimUzawaMkl : public OptimUzawaBase<OptimUzawaMkl<problem_t>, problem_t>
    {
      ...
      public:
         ...
         */
        template <std::size_t dim>
        void init_uzawa_impl(const scopi_container<dim>& particles, const std::vector<neighbor<dim>>& contacts);
        /**
         ...
         */
        template <std::size_t dim>
        void gemv_A_impl(const scopi_container<dim>& particles, const std::vector<neighbor<dim>>& contacts);
         ...
         */
        template <std::size_t dim>
        void gemv_transpose_A_impl(const scopi_container<dim>& particles, const std::vector<neighbor<dim>>& contacts);
    ...
    template <class problem_t>
    template <std::size_t dim>
    void OptimUzawaMkl<problem_t>::init_uzawa_impl(const scopi_container<dim>& particles, const std::vector<scopi::neighbor<dim>>& contacts)
    {
        ...
        m_status = mkl_sparse_convert_csr(A_coo, SPARSE_OPERATION_NON_TRANSPOSE, &m_A);
        ...
        PLOG_ERROR_IF(m_status != SPARSE_STATUS_SUCCESS && m_status != SPARSE_STATUS_NOT_SUPPORTED)
            << "Error in mkl_sparse_set_mv_hint for matrix A SPARSE_OPERATION_NON_TRANSPOSE: " << m_status;

        m_status = mkl_sparse_set_mv_hint(m_A, SPARSE_OPERATION_TRANSPOSE, m_descrA, 1);
        PLOG_ERROR_IF(m_status != SPARSE_STATUS_SUCCESS && m_status != SPARSE_STATUS_NOT_SUPPORTED)
            << " Error in mkl_sparse_set_mv_hint for matrix A SPARSE_OPERATION_TRANSPOSE: " << m_status;
    ...
    void OptimUzawaMkl<problem_t>::gemv_inv_P_impl(const scopi_container<dim>&)
    {
        ...
        PLOG_ERROR_IF(m_status != SPARSE_STATUS_SUCCESS && m_status != SPARSE_STATUS_NOT_SUPPORTED)
            << " Error in mkl_sparse_d_mv for U = - P^-1 * U: " << m_status;
    }

    template <class problem_t>
    template <std::size_t dim>
    void OptimUzawaMkl<problem_t>::gemv_A_impl(const scopi_container<dim>&, const std::vector<neighbor<dim>>&)
    {
        m_status = mkl_sparse_d_mv(SPARSE_OPERATION_NON_TRANSPOSE, -1., m_A, m_descrA, this->m_U.data(), 1., this->m_R.data()); // R = - A *
                                                                                                                                // U + R
        PLOG_ERROR_IF(m_status != SPARSE_STATUS_SUCCESS && m_status != SPARSE_STATUS_NOT_SUPPORTED)
            << " Error in mkl_sparse_d_mv for R = - A * U + R: " << m_status;
    }

    template <class problem_t>
    template <std::size_t dim>
    void OptimUzawaMkl<problem_t>::gemv_transpose_A_impl(const scopi_container<dim>&, const std::vector<neighbor<dim>>&)
    {
        m_status = mkl_sparse_d_mv(SPARSE_OPERATION_TRANSPOSE, 1., m_A, m_descrA, this->m_L.data(), 1., this->m_U.data()); // U = A^T * L +
                                                                                                                           // U
        PLOG_ERROR_IF(m_status != SPARSE_STATUS_SUCCESS && m_status != SPARSE_STATUS_NOT_SUPPORTED)
            << " Error in mkl_sparse_d_mv for U = A^T * L + U: " << m_status;
    }
        ...
        : base_type(nparts, dt)
    {
        ...
        PLOG_ERROR_IF(m_status != SPARSE_STATUS_SUCCESS && m_status != SPARSE_STATUS_NOT_SUPPORTED)
            << "Error in mkl_sparse_set_mv_hint for matrix invP: " << m_status;
    ...
    void OptimUzawaMkl<problem_t>::print_csr_matrix(const sparse_matrix_t A)
    {
        ...
        MKL_INT nbRows;
        MKL_INT nbCols;
        m_status = mkl_sparse_d_export_csr(A, &indexing, &nbRows, &nbCols, &csr_row_begin_ptr, &csr_row_end_ptr, &csr_col_ptr, &csr_val_ptr);
