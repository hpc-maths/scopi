...
namespace scopi
{
    ...
    auto closest_points(const superellipsoid<2, owner>& s1, const superellipsoid<2, owner>& s2)
    {
        ...
        neighbor<2, problem_t> neigh;
        // xt::xtensor_fixed<double, xt::xshape<2*(2*dim+dim-1+dim*dim)>> args = xt::hstack(xt::xtuple(
        xt::xtensor_fixed<double, xt::xshape<18>> args = xt::hstack(xt::xtuple(xt::view(s1.pos(), 0),
                                                                               s1.radius(),
                                                                               s1.squareness(),
                                                                               xt::flatten(s1.rotation()),
                                                                               xt::view(s2.pos(), 0),
                                                                               s2.radius(),
                                                                               s2.squareness(),
                                                                               xt::flatten(s2.rotation())));
        ...
        auto newton_GradF = [](auto u, auto args)
        {
            ...
            res(0, 0) = (-M01 * F1 - M00 * F3) * (M10 * F9 - M11 * F10) + (M11 * F1 + M10 * F3) * (M00 * F9 - M01 * F10)
                      + (-M01 * F12 + M00 * F11) * (-M10 * F4 - M11 * F2 - s1yc + N10 * F8 + N11 * F6 + s2yc)
                      + (M11 * F12 - M10 * F11) * (-M00 * F4 - M01 * F2 - s1xc + N00 * F8 + N01 * F6 + s2xc);
            res(0, 1) = (-M01 * F1 - M00 * F3) * (-N10 * F13 + N11 * F14) + (M11 * F1 + M10 * F3) * (-N00 * F13 + N01 * F14);
            res(1, 0) = ((M01 * F1 + M00 * F3) * (M01 * F12 - M00 * F11) + (M11 * F1 + M10 * F3) * (M11 * F12 - M10 * F11))
                          * std::sqrt(std::pow(N01 * F5 + N00 * F7, 2) + std::pow(N11 * F5 + N10 * F7, 2))
                          / std::sqrt(std::pow(M01 * F1 + M00 * F3, 2) + std::pow(M11 * F1 + M10 * F3, 2))
                      + (N01 * F5 + N00 * F7) * (M01 * F12 - M00 * F11) + (N11 * F5 + N10 * F7) * (M11 * F12 - M10 * F11);
            res(1, 1) = ((N01 * F5 + N00 * F7) * (N01 * F16 - N00 * F15) + (N11 * F5 + N10 * F7) * (N11 * F16 - N10 * F15))
                          * std::sqrt(std::pow(M01 * F1 + M00 * F3, 2) + std::pow(M11 * F1 + M10 * F3, 2))
                          / std::sqrt(std::pow(N01 * F5 + N00 * F7, 2) + std::pow(N11 * F5 + N10 * F7, 2))
    ...
    auto closest_points(const superellipsoid<3, owner>& s1, const superellipsoid<3, owner>& s2)
    {
        ...
        // xt::xtensor_fixed<double, xt::xshape<2*(2*dim+dim-1+dim*dim)>> args = xt::hstack(xt::xtuple(
        xt::xtensor_fixed<double, xt::xshape<34>> args = xt::hstack(xt::xtuple(xt::view(s1.pos(), 0),
                                                                               s1.radius(),
                                                                               s1.squareness(),
                                                                               xt::flatten(s1.rotation()),
                                                                               xt::view(s2.pos(), 0),
        ...
        auto newton_F = [](auto u, auto args)
        {
            ...
            res(3) = (M00 * A1 + M01 * A2 + M02 * A3) * (N00 * B4 + N01 * B5 + N02 * B6)
                   ...
                   + std::sqrt(std::pow(M00 * A1 + M01 * A2 + M02 * A3, 2) + std::pow(M10 * A1 + M11 * A2 + M12 * A3, 2)
                               + std::pow(M20 * A1 + M21 * A2 + M22 * A3, 2))
                         * std::sqrt(std::pow(N00 * B4 + N01 * B5 + N02 * B6, 2) + std::pow(N10 * B4 + N11 * B5 + N12 * B6, 2)
                                     + std::pow(N20 * B4 + N21 * B5 + N22 * B6, 2));
            return res;
        };
        auto newton_GradF = [](auto u, auto args)
        {
            ...
            res(0,
                0) = (-M10 * A1 - M11 * A2 - M12 * A3) * (M20 * A7 + M21 * A8 - M22 * A9)
            ...
            res(1,
                0) = (M00 * A1 + M01 * A2 + M02 * A3) * (M20 * A7 + M21 * A8 - M22 * A9)
            ...
            res(2,
                0) = (-M00 * A1 - M01 * A2 - M02 * A3) * (M10 * A7 + M11 * A8 - M12 * A9)
        ...
        // // exit(0);
        // // std::cout << " binit1_xy.size() = " << binit1_xy.size() << " ainit1_yz.size() = " << ainit1_yz.size() << " ainit1_xz.size() =
        // " << ainit1_xz.size() << std::endl; auto binit1 = xt::concatenate(xt::xtuple(binit1_xy,binit1_xy)); auto ainit1 =
        // xt::concatenate(xt::xtuple(ainit1_yz,ainit1_xz)); std::cout << " ainit1.size() = " << ainit1.size() << " binit1.size() = " <<
        // binit1.size() << std::endl;
        ...
        // // std::cout << " binit2_xy.size() = " << binit2_xy.size() << " ainit2_yz.size() = " << ainit2_yz.size() << " ainit2_xz.size() =
        // " << ainit2_xz.size() << std::endl; auto binit2 = xt::concatenate(xt::xtuple(binit2_xy,binit2_xy)); auto ainit2 =
        // xt::concatenate(xt::xtuple(ainit2_yz,ainit2_xz)); std::cout << " ainit2.size() = " << ainit2.size() << " binit2.size() = " <<
        // binit2.size() << std::endl;
        ...
        for (std::size_t i = 0; i < fl_agrid1.size(); i++)
        {
            ...
            {
                distances(i, j) = xt::linalg::norm(
                                      xt::flatten(s1.point(fl_agrid1(i), fl_bgrid1(i))) - xt::flatten(s2.point(fl_agrid2(j), fl_bgrid2(j))),
                                      2)
        ...
        //     std::cout << "l =" << l << " pt s1 = " << s1.point(fl_agrid1(xt::row(indmin,0)(l)),fl_bgrid1(xt::row(indmin,0)(l))) <<
        //     std::endl; std::cout << "l =" << l << " pt s2 = " <<
        //     s2.point(fl_agrid1(xt::row(indmin,1)(l)),fl_bgrid1(xt::row(indmin,1)(l))) << std::endl;
        // }
        ...
        xt::xtensor_fixed<double, xt::xshape<4>> u0 = {fl_agrid1(xt::row(indmin, 0)(0)),
                                                       fl_bgrid1(xt::row(indmin, 0)(0)),
                                                       fl_agrid2(xt::row(indmin, 1)(0)),
                                                       fl_bgrid2(xt::row(indmin, 1)(0))};
        // std::cout << "u0 =" << u0 << std::endl;
    ...
    {
        auto neigh = closest_points<problem_t>(s, seg);
        neigh.nij *= -1.;
        std::swap(neigh.pi, neigh.pj);
        return neigh;
    }
    ...
    auto closest_points(const sphere<3, owner>& s2, const superellipsoid<3, owner>& s1)
    {
        ...
        xt::xtensor_fixed<double, xt::xshape<30>> args = xt::hstack(xt::xtuple(xt::view(s1.pos(), 0),
                                                                               s1.radius(),
                                                                               s1.squareness(),
                                                                               xt::flatten(s1.rotation()),
                                                                               xt::view(s2.pos(), 0),
    ...
    {
        auto neigh = closest_points<problem_t>(s1, s2);
        neigh.nij *= -1.;
        std::swap(neigh.pi, neigh.pj);
        return neigh;
    }
    ...
    auto closest_points(const sphere<2, owner> s2, const superellipsoid<2, owner> s1)
    {
        ...
        xt::xtensor<double, 1> xts2r                   = {s2.radius()};
        xt::xtensor_fixed<double, xt::xshape<16>> args = xt::hstack(xt::xtuple(xt::view(s1.pos(), 0),
                                                                               s1.radius(),
                                                                               s1.squareness(),
                                                                               xt::flatten(s1.rotation()),
                                                                               xt::view(s2.pos(), 0),
        ...
        auto newton_F = [](auto u, auto args)
        {
            ...
            xt::xtensor_fixed<double, xt::xshape<2>> res;
            res(0) = -(M00 * D3 + M01 * D1) * (-M10 * D4 - M11 * D2 + N10 * D6 + N11 * D5 - s1yc + s2yc)
                   + (M10 * D3 + M11 * D1) * (-M00 * D4 - M01 * D2 + N00 * D6 + N01 * D5 - s1xc + s2xc);
            res(1) = (N00 * D6 + N01 * D5) * (M00 * D3 + M01 * D1) + (N10 * D6 + N11 * D5) * (M10 * D3 + M11 * D1)
        ...
        auto newton_GradF = [](auto u, auto args)
        {
            ...
            xt::xtensor_fixed<double, xt::xshape<2, 2>> res;
            res(0, 0) = (-M00 * D3 - M01 * D1) * (M10 * D7 - M11 * D8) + (M10 * D3 + M11 * D1) * (M00 * D7 - M01 * D8)
                      + (M00 * D9 - M01 * D10) * (-M10 * D4 - M11 * D2 + N10 * D6 + N11 * D5 - s1yc + s2yc)
                      + (-M10 * D9 + M11 * D10) * (-M00 * D4 - M01 * D2 + N00 * D6 + N01 * D5 - s1xc + s2xc);

            res(0, 1) = (-N00 * D5 + N01 * D6) * (M10 * D3 + M11 * D1) + (-N10 * D5 + N11 * D6) * (-M00 * D3 - M01 * D1);

            res(1, 0) = ((M00 * D3 + M01 * D1) * (-M00 * D9 + M01 * D10) + (M10 * D3 + M11 * D1) * (-M10 * D9 + M11 * D10))
                          * std::sqrt(std::pow(N00 * D6 + N01 * D5, 2) + std::pow(N10 * D6 + N11 * D5, 2))
                          / std::sqrt(std::pow(M00 * D3 + M01 * D1, 2) + std::pow(M10 * D3 + M11 * D1, 2))
                      + (N00 * D6 + N01 * D5) * (-M00 * D9 + M01 * D10) + (N10 * D6 + N11 * D5) * (-M10 * D9 + M11 * D10);

            res(1, 1) = ((-N00 * D5 + N01 * D6) * (N00 * D6 + N01 * D5) + (-N10 * D5 + N11 * D6) * (N10 * D6 + N11 * D5))
                          * std::sqrt(std::pow(M00 * D3 + M01 * D1, 2) + std::pow(M10 * D3 + M11 * D1, 2))
                          / std::sqrt(std::pow(N00 * D6 + N01 * D5, 2) + std::pow(N10 * D6 + N11 * D5, 2))
                      + (-N00 * D5 + N01 * D6) * (M00 * D3 + M01 * D1) + (-N10 * D5 + N11 * D6) * (M10 * D3 + M11 * D1);
            return res;
        };
    ...
    {
        auto neigh = closest_points<problem_t>(sphere, superellipsoid);
        neigh.nij *= -1.;
        std::swap(neigh.pi, neigh.pj);
        return neigh;
    }
    ...
    auto closest_points(const superellipsoid<3, owner> s1, const plan<3, owner> p2)
    {
        ...
        auto newton_F = [](auto u, auto args)
        {
            ...
            res(0) = -(M10 * A1 + M11 * A2 + M12 * A3) * (-M20 * A4 - M21 * A5 - M22 * A6 + N21 * a2 + N22 * b2 + p2zc - s1zc)
                   + (M20 * A1 + M21 * A2 + M22 * A3) * (-M10 * A4 - M11 * A5 - M12 * A6 + N11 * a2 + N12 * b2 + p2yc - s1yc);

            res(1) = (M00 * A1 + M01 * A2 + M02 * A3) * (-M20 * A4 - M21 * A5 - M22 * A6 + N21 * a2 + N22 * b2 + p2zc - s1zc)
                   - (M20 * A1 + M21 * A2 + M22 * A3) * (-M00 * A4 - M01 * A5 - M02 * A6 + N01 * a2 + N02 * b2 + p2xc - s1xc);

            res(2) = -(M00 * A1 + M01 * A2 + M02 * A3) * (-M10 * A4 - M11 * A5 - M12 * A6 + N11 * a2 + N12 * b2 + p2yc - s1yc)
                   + (M10 * A1 + M11 * A2 + M12 * A3) * (-M00 * A4 - M01 * A5 - M02 * A6 + N01 * a2 + N02 * b2 + p2xc - s1xc);

            res(3) = N00 * sign_p2s * (M00 * A1 + M01 * A2 + M02 * A3) + N10 * sign_p2s * (M10 * A1 + M11 * A2 + M12 * A3)
        ...
        auto newton_GradF = [](auto u, auto args)
        {
            ...
            res(3, 0) = N00 * sign_p2s * (-M00 * A10 - M01 * A11 + M02 * A12) + N10 * sign_p2s * (-M10 * A10 - M11 * A11 + M12 * A12)
            ...
            res(3, 1) = N00 * sign_p2s * (-M00 * A15 + M01 * A16) + N10 * sign_p2s * (-M10 * A15 + M11 * A16)
    ...
    auto closest_points(const superellipsoid<2, owner> s1, const plan<2, owner> d2)
    {
        ...
        auto newton_GradF = [](auto u, auto args)
        {
            ...
            res(0, 0) = (-M00 * A1 - M01 * A2) * (M10 * A13 - M11 * A14) + (M10 * A1 + M11 * A2) * (M00 * A13 - M01 * A14)
            ...
            res(1, 0) = N00 * sign_d2s * (-M00 * A15 + M01 * A16) + N10 * sign_d2s * (-M10 * A15 + M11 * A16)
                      + std::sqrt(std::pow(N00, 2) + std::pow(N10, 2))
    ...
    struct closest_points_functor
    {
         ...
         */
        using return_type = neighbor<dim, problem_t>;
        ...
        {
            return closest_points<problem_t>(obj1, obj2);
        }
        ...
        {
            return closest_points<problem_t>(obj1, obj2, i1, i2);
        }
     ...
     * @tparam ower
     */
    template <class problem_t, std::size_t dim, bool owner = false>
