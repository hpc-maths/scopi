...
namespace scopi
{
    ...
    void add_contact_from_object(const worm<dim, false>& w, std::size_t offset, Contacts& contacts)
    {
        ...
        {
            auto neigh = closest_points_dispatcher<dim>::dispatch(*w.get_sphere(i), *w.get_sphere(i+1));
            neigh.i = offset + i;
            neigh.j = offset + i + 1;
            neigh.nij *= -1;
            neigh.dij *= -1;
    ...
    template <std::size_t dim>
    using add_contact_from_object_dispatcher = unit_static_dispatcher
    <
        add_contact_from_object_functor<dim>,
