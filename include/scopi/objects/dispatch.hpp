...
namespace scopi
{
    ...
    class double_static_dispatcher
    {
      private:
        ...
        static return_type invoke_executor(lhs_type& lhs, rhs_type& rhs, std::false_type, Args&&... args)
        {
            executor exec;
        ...
        template <class lhs_type, class rhs_type, class... Args>
        static return_type invoke_executor(lhs_type& lhs, rhs_type& rhs, std::true_type, Args&&... args)
        {
            executor exec;
        ...
        template <class lhs_type, class... Args>
        static return_type dispatch_rhs(lhs_type& lhs, base_rhs& rhs, mpl::vector<>, Args&&... args)
        {
            executor exec;
        ...
        static return_type dispatch_rhs(lhs_type& lhs, base_rhs& rhs, mpl::vector<T, U...>, Args&&... args)
        {
            if (T* p = dynamic_cast<T*>(&rhs))
            {
                ...
                using invoke_flag = std::integral_constant<bool, std::is_same<symmetric, symmetric_dispatch>::value && (rhs_index < lhs_index)>;
        ...
        template <class... Args>
        static return_type dispatch_lhs(base_lhs& lhs, base_rhs& rhs, mpl::vector<>, Args&&... args)
        {
            executor exec;
            return exec.on_error(lhs, rhs, std::forward<Args>(args)...);
        }
        ...
        template <class T, class... U, class... Args>
        static return_type dispatch_lhs(base_lhs& lhs, base_rhs& rhs, mpl::vector<T, U...>, Args&&... args)
        {
            if (T* p = dynamic_cast<T*>(&lhs))
            {
