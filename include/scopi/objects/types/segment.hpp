...
namespace scopi
{
    ...
    auto segment<dim, owner>::extrema() const
    {
        ...
        pts[0] = xt::linalg::dot(rotation_matrix<dim>(this->q(0)), xt::xtensor_fixed<double, xt::xshape<dim>>{0, -0.5 * m_length})
               + this->pos(0);
        pts[1] = xt::linalg::dot(rotation_matrix<dim>(this->q(0)), xt::xtensor_fixed<double, xt::xshape<dim>>{0, 0.5 * m_length})
               + this->pos(0);
        return pts;
    }
