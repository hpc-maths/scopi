...
namespace soa
{
    ...
    inline vector<T>::vector() : detail::members_with_size<T>{}
    {
        ...
        this->capacity_ = get_span<0>().capacity();
    }
    ...
    template<class T>
    inline auto vector<T>::capacity() const noexcept -> size_type
    {
    ...
    void vector<T>::push_back(T const &value)
    {
        ...
        detail::for_each(
            detail::as_tuple(base()), tuple, sequence_type{},
            [](auto &span, auto &value) { span.push_back(value); });
        this->size_ = get_span<0>().size();
        this->capacity_ = get_span<0>().capacity();
    }
...
#define SOA_PP_MEMBER(nb, type, name)                                          \
    vector_span<std::vector<decltype(std::declval<type>().name)>> name;
...
#define SOA_DEFINE_TYPE(type, ...)                                             \
    ...
    {                                                                          \
        ...
        struct members<::type>                                                 \
        {                                                                      \
            using container_base = std::vector<char>;                          \
