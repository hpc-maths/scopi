...
namespace scopi
{
                                ...
                                public ViscousBase<dim>
    {
      public:
         ...
         *  - one element corresponding to \f$ D > 0 \f$ and a second element corresponding to \f$ D < 0 \f$, after all the other
         * constraints, if \f$ \gamma_{\min} < \gamma_{ij} < 0 \f$.
         *
         *  In other words, \f$ d \f$ is a block vector like
         *  \f[
         *      \mathbf{d} = \left( \text{mix of blocks with one or four elements corresponding to } D > 0  \text{ or to the friction model,
         * blocks of one element corresponding to } D < 0 \right). \f]
         *
         * @tparam dim Dimension (2 or 3).
         * @param contacts [in] Array of contacts.
         */
        ...
        /**
         * @brief Set \f$ \gamma_{ij}^n \f$ from the previous time step, compute the number of contacts with \f$ \gamma_{ij} < 0 \f$ and \f$
         * \gamma_{ij} < \gamma_{\min} \f$.
         *
         ...
         *
         * \f[
         *      \gamma^{n+1}_{ij} = \max \left( \gamma_{\min}, \gamma^n_{ij} - \Delta t \left( \mathbf{\lambda}_{ij}^+ -
         * \mathbf{\lambda}_{ij}^- \right) \right). \f]
         *
         * @param contacts [in] Array of contacts.
         ...
         */
        template <class optim_solver_t>
        void extra_steps_after_solve(const contact_container_t& contacts, optim_solver_t& solver);
        /**
      ...
      private:
         ...
         * @param u
         */
        void correct_lambda(const contact_container_t& contacts,
    ...
    template <std::size_t dim>
    void ViscousWithFriction<dim>::create_matrix_constraint_coo(const scopi_container<dim>& particles, const contact_container_t& contacts)
    {
        // Set up A matrix
                    ...
                    for (std::size_t ind_row = 0; ind_row < 3; ++ind_row)
                    {
                        for (std::size_t ind_col = 0; ind_col < 3; ++ind_col)
                        {
                            ...
                            this->m_A_values.push_back(
                                -this->m_params.mu * this->m_dt * dot(ind_row, ind_col)
                                + this->m_params.mu * this->m_dt * c.nij[ind_row]
    ...
    template <std::size_t dim>
    template <class optim_solver_t>
    void ViscousWithFriction<dim>::extra_steps_before_solve(const contact_container_t& contacts_new, optim_solver_t&)
    {
    ...
    template <std::size_t dim>
    void ViscousWithFriction<dim>::correct_lambda(const contact_container_t& contacts,
                                                  xt::xtensor<double, 1> lambda,
                                                  const scopi_container<dim>& particles,
                                                  const xt::xtensor<double, 2>& u)
    {
        ...
        for (std::size_t ic = 0; ic < contacts.size(); ++ic)
        {
            ...
            else
            {
                ...
                {
                    m_lambda[ic] = +xt::linalg::dot(xt::view(u, contacts[ic].j, xt::range(_, dim)) - particles.v()(contacts[ic].j),
                                                    contacts[ic].nij)(0)
                                 / this->m_dt;
                }
                ind_gamma_min++;
    ...
    template <std::size_t dim>
    template <class optim_solver_t>
    void ViscousWithFriction<dim>::extra_steps_after_solve(const contact_container_t& contacts, optim_solver_t& optim_solver)
    {
        if (contacts.size() == 0)
        {
        ...
        if (m_nb_gamma_min != 0)
        {
            ...
            this->m_should_solve = (xt::linalg::norm(m_s_old - m_s) / (xt::linalg::norm(m_s) + 1.) > this->m_params.tol_fixed_point
                                    && m_nb_iter < this->m_params.max_iter_fixed_point);
        }
        else
        {
            this->m_should_solve = false;
            ...
            for (std::size_t i = 0; i < this->m_gamma_old.size(); ++i)
            {
                ...
                else
                {
                    f_contact = lambda(this->m_gamma.size() - m_nb_gamma_min + this->m_nb_gamma_neg + 4 * ind_gamma_min)
                              - lambda(this->m_gamma.size() - m_nb_gamma_min + ind_gamma_neg);
                    ind_gamma_neg++;
                    ind_gamma_min++;
                }
    ...
    template <std::size_t dim>
    std::size_t ViscousWithFriction<dim>::number_row_matrix(const contact_container_t& contacts) const
    {
        return contacts.size() - m_nb_gamma_min + this->m_nb_gamma_neg + 4 * m_nb_gamma_min;
    }

    template <std::size_t dim>
    void ViscousWithFriction<dim>::create_vector_distances(const contact_container_t& contacts)
    {
