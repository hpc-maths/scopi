...
namespace scopi
{
    ...
    std::pair<type::position_t<2>, double> analytical_solution_sphere_plan_viscous(double alpha,  double t, double r, double g, double y0, double gamma_min, double t_inv)
    {
        ...
        {
            x[1] = r;
            x[0] = g*std::pow(t_inv,2)*std::sin(alpha)/2+g*t_inv*std::sin(alpha)*(t-t_inv)-g*pow((t-t_inv),2)*std::sin(alpha)/2;
            return std::make_pair(x, 0.);
        }
        else
        {
    ...
    std::pair<type::position_t<2>, double> analytical_solution_sphere_plan_friction(double alpha, double mu, double t, double r, double g, double y0)
    {
        ...
        if (t > t_impact)
        {
            ...
            double xt_impact = g*std::pow(t_impact,2)*std::sin(alpha)/2;
        ...
        else if (t<t_inv)
        {
            ...
            if(std::tan(alpha) <= 3*mu)
            {
                ...
                if(t>t_no_slide)
                {
                    double vt_no_slide = g*(std::sin(alpha) - mu*std::cos(alpha))*(t_no_slide-t_gamma_min) + v_t_gamma_min;
                    double omega_no_slide = -2.*mu*g*std::cos(alpha)*(t_no_slide-t_gamma_min)/r;
                    double xt_no_slide = g*(std::sin(alpha) - mu*std::cos(alpha))*(std::pow(t_no_slide-t_gamma_min,2))/2 + v_t_gamma_min*(t_no_slide-t_gamma_min)+x_t_gamma_min;
                    double angle_no_slide = -2.*mu*g*std::cos(alpha)*(std::pow(t_no_slide-t_gamma_min,2))/(2*r);
                    x[0] = 2.*g*std::sin(alpha)*(std::pow(t-t_no_slide,2))/6. + vt_no_slide*(t-t_no_slide)+xt_no_slide;
                    angle = -2.*g*std::sin(alpha)*(std::pow(t-t_no_slide,2))/(6.*r) + omega_no_slide*(t-t_no_slide)+angle_no_slide;
                }
                else
                {
        ...
        else
        {
            ...
            if(std::tan(alpha) <= 3*mu)
            {
                ...
                if(t_inv>t_no_slide)
                {
                    ...
                    double angle_no_slide = -2.*mu*g*std::cos(alpha)*(std::pow(t_no_slide-t_gamma_min,2))/(2*r);
                    vt_tinv = 2.*g*std::sin(alpha)*(t_inv-t_no_slide)/3. + vt_no_slide;
                    x_tinv = 2.*g*std::sin(alpha)*(std::pow(t_inv-t_no_slide,2))/6. + vt_no_slide*(t_inv-t_no_slide)+xt_no_slide;
                    omega_tinv = -2.*g*std::sin(alpha)*(t_inv-t_no_slide)/(3.*r) + omega_no_slide;
                    angle_tinv = -2.*g*std::sin(alpha)*(std::pow(t_inv-t_no_slide,2))/(6.*r) + omega_no_slide*(t_inv-t_no_slide)+angle_no_slide;
                }
                else
                {
                    vt_tinv = g*(std::sin(alpha) - mu*std::cos(alpha))*t2 + v_t_gamma_min;
