...
namespace scopi
{
    ...
    std::pair<type::position_t<2>, double> analytical_solution_sphere_plane_no_friction(double alpha, double t, double r, double g, double y0)
    {
        const double t_impact = std::sqrt(2 * (y0 - r) / (g * std::cos(alpha)));
        type::position_t<2> x;
        if (t < t_impact)
    ...
    std::pair<type::position_t<2>, double>
    analytical_solution_sphere_plane_viscous_friction(double alpha, double mu, double t, double r, double g, double y0, double gamma_min, double t_inv)
    {
        double angle;
        double t_impact    = std::sqrt(2 * (y0 - r) / (g * std::cos(alpha)));
        double t_gamma_min = -gamma_min / (g * std::cos(alpha));
